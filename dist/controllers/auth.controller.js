...
exports.syncUser = exports.getMe = exports.deleteAccount = exports.adminRegister = exports.handlerLogin = exports.adminLogin = exports.logout = exports.login = exports.register = void 0;
...
const register = async (req, res, next) => {
    try {
        ...
        if (data.user) {
            ...
            const { error: dbError } = await supabase_1.supabaseAdmin.from('users').insert({
                id: data.user.id,
...
const deleteAccount = async (req, res, next) => {
    try {
        ...
        const { error: authError } = await supabase_1.supabaseAdmin.auth.admin.deleteUser(user.id);
        if (authError) {
            res.status(400).json({ success: false, error: authError.message });
            return;
        }
        // Delete user from public.users table
        await supabase_1.supabaseAdmin.from('users').delete().eq('id', user.id);
